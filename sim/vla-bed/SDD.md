...
**Lower expert noise (recipe v5a, 6 Sep 2026).** The Geometric-Entropy result (2606.20871)
predicts that injected trajectory diversity hurts a *fine-tuned* VLA, so v5a keeps v4's
headroom and camera jitter and halves the DART noise to 0.25 × the limits (2.5 mm, 0.0125 rad).
Recorded: 100 % success, 0 faults, episodes 33.0 / 33.6 frames (v4 35.5 / 35.9), gate PASS.
**Measured (sessions F+G, 6 Sep 2026, $0; 10k steps at 0.917 steps/s, 3.04 h; evaluation
1.72 h, `results/p5/8af08da6f6a2`):** nominal 2.5k 0.11 / 5k 0.09 / 7.5k 0.16 [0.10, 0.24] /
10k 0.10, selected 7.5k (R11); every v4 → v5a checkpoint pair is inside the noise (+0.04,
−0.04, +0.07, +0.01; McNemar p 0.19–1) and best-vs-best v3 5k → v5a 7.5k is −0.04
[−0.14, +0.06] (p = 0.54), so the prediction is not confirmed at n = 100 and v5b (noise 0) is
registered but not recorded. Under the shifted camera the 10k policy scores **0.21**
[0.14, 0.30]: above its own nominal (+0.11 [+0.02, +0.20], p = 0.035), above v3 (+0.16
[+0.08, +0.24], 18 vs 2 discordant, p = 0.0004) and v4 (+0.08 [0.00, +0.16], p = 0.077).
`camera_shift_far` 0.05, lighting 0.09 and target_relocation 0.11 are within noise of v4; the
gain-0.61 probe adds less (0.15 vs 0.10, p = 0.23) because the policy already commands
0.005 m steps. v5a is the base recipe for the plastic, v6 and v7 runs.

**Wrist camera (recipe v6, 6 Sep 2026).** The Curse-of-Precision ablation (2607.23108) moves
the precision floor from 2.35 to 3.85 mm when the wrist camera is removed, and the precision
curves (`precision.py`) show every recipe's misses are centimetre-scale, so v6 = v5a plus a
second image stream `observation.images.wrist` from a camera on `wrist_3_link` (0.10 m off
the tool axis, tilted 21° so the optical axis meets the tool axis at the pinch site; fovy 70°,
...
step on the T4 (1.8 × v5a), 10k steps ≈ 5.4 h inside the 6 h guard.
**Measured (sessions I+K, 6–7 Sep 2026, $0; 10k steps at 0.513 steps/s, 5.43 h, 7.1 GB VRAM, loss
0.589; evaluation 1.66 h, `results/p5/bd883c780fba`):** nominal 2.5k 0.80 / 5k 0.87 / 7.5k 0.89 /
10k **0.89 [0.81, 0.94]**, selected 10k (R11; 5k and 7.5k within its CI). Paired on the identical
seeds, v5a → v6 on 10k is +0.79 [+0.71, +0.87] with 79 vs 0 discordant pairs (McNemar p = 3 × 10⁻²⁴),
and every earlier best (v2 7.5k, v3 5k at 0.20) → v6 is +0.69 [+0.58, +0.79] (p = 4 × 10⁻¹⁸); the
2.5k checkpoint alone (0.80) is above any single-camera number, so the gain is not a selection
effect. The variations move with it: camera_shift 0.86 (v5a 0.21; not separable from v6's own
nominal, p = 0.63), lighting 0.85, target_relocation 0.90 (both inside the noise of nominal),
camera_shift_far 0.75 (v5a 0.05; −0.14 against nominal, p = 0.004, still the weakest view), blank
image 0.05 (the policy uses the images), rejected steps 0.3 %, mean episode 46.6 frames (oracle
33.6). The gain-0.61 probe that helped every single-camera recipe now hurts (0.75, −0.14, p =
0.004): the policy no longer overshoots. Reading: at a 30 mm acceptance radius that every
single-camera recipe missed by centimetres (§3e precision curves), the eye-in-hand view removes
the depth ambiguity of one third-person 224² image (2606.12334) and, in the direction of the
Curse-of-Precision ablation (2607.23108), turns a 0.1–0.2 policy into a 0.9 policy at the same
demonstration count and 1.8 × the training compute. v6 is the base recipe for v7 (DAgger) and the
gripper / chunkwise variants.
