...
- download vla-bed-baseline-eval.zip (sha256 4118f3f7…8645e9737) and vla-bed-baseline-probes.zip (sha256 ff3e1a2e…aa84955b) from the Kaggle Output tabs and run sim/vla-bed/gpu/kaggle_import.sh on each (replaces these transcribed numbers with the packed per-episode JSONs)
- session B: train baseline on recipe v3 (headroom 0.7 + camera jitter) and evaluate it on the same frozen suite, paired against baseline-v2
