...
| `lerobot/berkeley_autolab_ur5` | dataset revision at P3 | CC-BY-4.0 | Berkeley AUTOLAB (Chen, Adebola, Goldberg); conversion by LeRobot | selected episodes downloaded for replay and optional co-training | Attribution: cite `BerkeleyUR5Website`, `lerobot_berkeley_autolab_ur5` and the OXE paper. Replay videos and co-trained checkpoints are derivatives and state "derived from the Berkeley UR5 Demonstration Dataset (CC-BY-4.0)". |
