...
# Run AFTER scripts/launch/simulation/turtlebot.sh, in its own terminal.
#
#   scripts/launch/simulation/nav2.sh [path/to/map.yaml]      # default: ~/map.yaml
#
...
if [[ ! -f "$MAP" ]]; then
  echo "No map at '$MAP'. Build one first with scripts/launch/simulation/slam.sh, then:"
