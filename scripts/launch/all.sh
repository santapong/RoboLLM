...
open_term "TB3 sim" "$ROOT/scripts/launch/simulation/turtlebot.sh"
...
open_term "dashboard" "$ROOT/scripts/launch/dashboard.sh"

if [[ -n "$NAV_MAP" ]]; then
  sleep 2
  echo "Launching Nav2 with map: $NAV_MAP"
  open_term "nav2" "$ROOT/scripts/launch/simulation/nav2.sh '$NAV_MAP'"
fi
...
Not started here (run when you want them):
  • SLAM / build a map :  $ROOT/scripts/launch/simulation/slam.sh
  • Robot arm (MoveIt) :  $ROOT/scripts/launch/simulation/moveit_panda.sh
EOF
