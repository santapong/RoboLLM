#!/usr/bin/env python3
"""
RoboLLM MCP application — a tiny, readable ROS 2 <-> MCP bridge.

Exposes a handful of tools over MCP (stdio) so an LLM client (Claude Code) can
introspect and drive the robot. The actual ROS 2 node lives in
``src/robollm/bridge.py`` and is shared with the dashboard application.
...
Requires ROS 2 (Jazzy) sourced before launch — see scripts/launch/mcp.sh.
"""
...
from robollm import gazebo_world
from robollm.bridge import get_bridge
...
def move_arm(joints: str, group: str = "panda_arm", timeout_s: float = 60.0) -> str:
    ...
    e.g. scripts/launch/simulation/moveit_panda.sh). `joints` = comma-separated name=radians:
