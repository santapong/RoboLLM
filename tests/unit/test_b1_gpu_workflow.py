...
GPU_SCRIPTS = ROOT / "scripts" / "learning" / "b1" / "gpu"
PREFLIGHT_PATH = GPU_SCRIPTS / "preflight.py"
...
def test_training_configuration_is_frozen_local_only_and_compatible():
    ...
    config = module.load_config(ROOT / "configs" / "training" / "b1_smolvla.json")
...
def test_smoke_and_full_training_default_to_dry_run():
    for mode in ("smoke", "full"):
        process = subprocess.run(
            [sys.executable, str(GPU_SCRIPTS / "train.py"), mode],
            check=True,
            capture_output=True,
            text=True,
        )
...
def test_checkpoint_selector_considers_only_frozen_steps(tmp_path):
    ...
    process = subprocess.run(
        [
            sys.executable,
            str(GPU_SCRIPTS / "select_checkpoint.py"),
            "--results-root",
            str(results),
        ],
        check=True,
