...
conversions. constraints.txt MUST keep the exact pin, and any numpy that happens
...
_CONSTRAINTS = os.path.join(_ROOT, "constraints.txt")
...
def test_constraints_pins_numpy_1_26_4():
    ...
    assert re.search(r"(?m)^\s*numpy==1\.26\.4\s*$", text), (
        "constraints.txt must pin numpy==1.26.4 (ROS 2 Jazzy ABI law)"
    )
