...
plumbing of its own — it calls `get_bridge("claude_web_bridge")` from the root
`robot_bridge.py`, the same shared rclpy node code the MCP server uses, so a
...
- **`run-web.sh`** — sources `/opt/ros/jazzy/setup.bash` (+ `~/ros2_ws` overlay
  if present), defaults `TURTLEBOT3_MODEL=burger` and `HOST=127.0.0.1`, then
  execs `.venv/bin/python -m uvicorn web.server:app --host $HOST --port 8080`
  from the repo root.
- **`robot_bridge.py`** (repo root) — publishes `/cmd_vel`; a 20 Hz timer
...
| `run-web.sh` | launcher: ROS env + venv + uvicorn on :8080 |
| `../robot_bridge.py` | shared rclpy node (`get_bridge()`), deadman + safe-mode teleop |
...
```bash
web/run-web.sh                     # → http://localhost:8080  (Ctrl-C to stop)
HOST=0.0.0.0 ROBOT_TOKEN=secret web/run-web.sh   # LAN mode with auth

curl -s localhost:8080/api/topics | head -c 300   # JSON topic list = server up
curl -s -o /dev/null -w '%{http_code}\n' localhost:8080/api/camera  # 204 without camera
```

With `sim/launch_turtlebot.sh` running, the header dot turns green (WS
...
- Editing `robot_bridge.py` while the dashboard runs requires restarting
